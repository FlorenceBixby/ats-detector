...
// ../../.wrangler/tmp/pages-3NLgGU/functionsWorker-0.7822555320943646.mjs
...
function detectATS(html, url) {
  ...
  let match2;
  while ((match2 = hrefRegex.exec(html)) !== null) {
    const href = match2[1];
    for (const { pattern, name } of ATS_PATTERNS) {
  ...
  for (const { pattern, name } of ATS_PATTERNS) {
    if (!results.has(name) && pattern.test(html)) {
      const match3 = html.match(new RegExp(`https?://[^"' <>]*${pattern.source}[^"' <>]*`, "i"));
      results.set(name, match3 ? match3[0] : null);
    }
  }
  return Array.from(results.entries()).map(([name, url2]) => ({ name, url: url2 }));
}
__name(detectATS, "detectATS");
__name2(detectATS, "detectATS");
  ...
  for (const url of urlsToCheck) {
    try {
      ...
      for (const match2 of matches) {
        if (!allMatches.has(match2.name)) {
          allMatches.set(match2.name, match2.url);
        }
      }
...
// .wrangler/tmp/bundle-bJujVR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bJujVR/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7822555320943646.js.map
